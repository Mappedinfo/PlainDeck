...
const colorRoles = ['background', 'text', 'muted', 'accent'] as const
...
export function applyDocumentTheme(input: DeckDocument, nextTheme: Theme): DeckDocument {
  ...
  const replace = (value?: string) => {
    ...
    const role = colorRoles.find(key => previous[key].toLowerCase() === value.toLowerCase())
    return role ? nextTheme.colors[role] : value
  }
