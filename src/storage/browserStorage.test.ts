...
describe('browser project storage', () => {
  ...
  it('uses the shared commit order and removes deleted slide files', async () => {
    ...
    expect(events).toEqual(['write:slides/006-browser-safe.json', 'write:deck.json'])
    events.length = 0
    const removed = applyOperations(added.document, [{ op: 'remove-slide', slide: './slides/006-browser-safe.json' }])
    await writeProject(root, removed.document, new Set(removed.changedPaths))
    expect(events).toEqual(['write:deck.json', 'remove:slides/006-browser-safe.json'])
    expect((await readProject(root)).deck.slides).not.toContain('./slides/006-browser-safe.json')
  })
