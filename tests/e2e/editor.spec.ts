...
test('creates an editable local summary-card page from structured Markdown', async ({ page }) => {
  ...
  await expect(dialog).toContainText('174 NATIVE RECIPES')
  await expect(dialog).toContainText('已识别 4 个要点')
  await dialog.getByRole('textbox', { name: '搜索设计配方' }).fill('terminalCli')
  await expect(dialog.locator('.style-recipe-list > button')).toHaveCount(1)
  await dialog.locator('.style-recipe-list > button').click()
